...
date: '2025. 06. 06'
thumbnail: '/post/react-context-api/thumbnail.webp'
---

import { Outline } from '@/widget/outline';

<Outline
  content={`이 글에서는 React Context API의 핵심 목적인 의존성 주입 패턴을 통해 코드 품질을 높이는 방법을 소개한다. 나아가 Context API를 활용하여 상태 관리 시 발생하는 리렌더링 문제를 Zustand와 결합해 최적화하는 구체적인 패턴까지 함께 제안한다.`}
/>
